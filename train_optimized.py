...
data_tag = '4wavelet_'
# user should specify the directories for data, coil, mask directory
data_dir = "..."
coil_dir = "..."
mask_dir = '...'
...
class UnrolledNet():
    def __init__(self, input_x, sens_maps, mask, nb_blocks, training):
...
nw_out, ul_output, x0, all_intermediate_outputs, lam, scaling_factor, etta = UnrolledNet(atbT, csmT, maskT, nb_blocks,
...
trnOrg, trnAtb = c2r(trnOrg), c2r(trnAtb)
...
with tf.device('/cpu:0'):
    ...
    # creating the dataset
    dataset = tf.data.Dataset.from_tensor_slices((kspaceP, orgP, atbP, csmP, maskP))
    ...
    for i in range(len(num_gpus)):
        with tf.device(assign_to_device('/gpu:{}'.format(num_gpus[i]), ps_device='/cpu:0')):
            kspaceT, orgT, atbT, csmT, maskT = iterator.get_next('getNext')
            # %% make training model
            out, ul_output, cg_out, all_intermediate_results, lam_out, scaling_factor_out, etta_out = UnrolledNet(atbT, csmT, maskT, nb_blocks,
    ...
    with tf.Session(config=config) as sess:
        ...
        feedDict = {kspaceP: kspace_train_ref, orgP: trnOrg, atbP: trnAtb, maskP: trnMask, csmP: trnCsm}
